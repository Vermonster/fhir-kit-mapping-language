...
class Rule {
  ...
  transform (variableMap, transformContext) {
    const { ruleTargets, ruleSources, dependent } = this;

    transformContext.rules.unshift(this);
    if (ruleSources) {
      ruleSources.forEach(ruleSource => ruleSource.addVariableMap(variableMap));
    }

    if (ruleTargets) {
      ruleTargets.forEach(ruleTarget => ruleTarget.addVariableMap(variableMap));
      ruleTargets.forEach(ruleTarget => ruleTarget.transformTarget(variableMap, transformContext));
    }
